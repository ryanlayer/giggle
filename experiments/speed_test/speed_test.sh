...
if [ "$(uname)" == "Darwin" ]; then
	...
	/usr/bin/time -f "%e real\t%U user\t%S sys" bedtools/bedtools_many_search.sh data/split_s$S/ data/r$R.sort.bed  data/rme.human.hg19.genome > /dev/null
fi
...
if [ "$(uname)" == "Darwin" ]; then
	...
	/usr/bin/time -f "%e real\t%U user\t%S sys" tabix/tabix_many_search.sh data/split_s$S/ data/r$R.sort.bed > /dev/null
fi
...
if [ "$(uname)" == "Darwin" ]; then
	...
	/usr/bin/time -f "%e real\t%U user\t%S sys" giggle search -i data/split_s$S\_b/ -q data/r$R.sort.bed.gz > /dev/null
fi
