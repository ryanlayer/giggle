...
int search_help(int exit_code) {
  fprintf(stderr,
          ...
          "             -r <regions (CSV)>\n"
          "             -q <query file>\n"
          "             -o give results per record in the query file (omits "
          "empty results)\n"
          "             -c give counts by indexed file\n"
          "             -s give significance by indexed file (requires query "
          "file)\n"
          "             -v give full record results\n"
          "             -f print results for files that match a pattern (regex "
          "CSV)\n"
          "             -g genome size for significance testing (default "
          "3095677412)\n"
...
int print_giggle_query_result(struct giggle_query_result *gqr,
  ...
  for (i = 0; i < gqr->num_files; i++) {
    ...
    if (test_pattern_match(gi, regexs, file_patterns, num_file_patterns, i,
                           f_is_set)) {
      if ((v_is_set == 1) && (giggle_get_query_len(gqr, i) > 0)) {
        ...
        printf("#%s\t"
               "size:%u\t"
               "overlaps:%u\n",
               fd->file_name, fd->num_intervals, giggle_get_query_len(gqr, i));
      } else if (s_is_set == 1) {
        ...
        long long n21 =
            (long long)safe_subtract(fd->num_intervals, file_counts);

        double comp_mean = fd->mean_interval_size + mean_interval_size;

        long long n22_full =
            (long long)MAX(n11 + n12 + n21, genome_size / comp_mean);
        ...
        long double r =
            _kt_fisher_exact(n11, n12, n21, n22, &left, &right, &two);
        ...
        */
        ...
        double ratio = (((double)(n11 + 1) / (double)MAX(1, n12)) /
        ...
        printf("%s\t"
               ...
               "%.17Lg\t"
               "\n",
               fd->file_name, fd->num_intervals, file_counts, ratio, two, left,
               right, log2fc(ratio) * neglog10p(two));
        /*
        printf("#%s\t"
               "size:%u\t"
               "overlaps:%u\t"
...
int search_main(int argc, char **argv, char *full_cmd) {
  ...
  int c;
  char *index_dir_name = NULL, *regions = NULL, *query_file_name = NULL,
       *file_patterns_to_be_printed = NULL, *query_filter_string = NULL;

  char *i_type = "i";

  int i_is_set = 0, l_is_set = 0, r_is_set = 0, q_is_set = 0, c_is_set = 0,
      s_is_set = 0, v_is_set = 0, f_is_set = 0, o_is_set = 0, m_is_set = 0,
      u_is_set = 0;
  ...
  if (l_is_set == 1) {
    ...
    uint32_t num_files = giggle_get_indexed_files(
        index_dir_name, &names, &num_intervals, &mean_interval_sizes);
    uint32_t i;
    printf("File name\tNumber of intervals\tMean interval size\n");
    for (i = 0; i < num_files; ++i) {
      printf("%s\t%u\t%lf\n", names[i], num_intervals[i],
  ...
  if (f_is_set == 1) {
    ...
    while (scan_s(file_patterns_to_be_printed,
                  strlen(file_patterns_to_be_printed), &s, &e, ',') >= 0) {
      num_file_patterns += 1;
      s = e + 1;
    }
    ...
    regexs = (regex_t *)malloc(num_file_patterns * sizeof(regex_t));
    if (regexs == NULL)
      err(1, "malloc error  in search_main().");

    file_patterns = (char **)malloc(num_file_patterns * sizeof(char *));
    ...
    while (scan_s(file_patterns_to_be_printed,
      ...
      if (r != 0) {
        errx(EX_USAGE, "Could not compile regex '%s'", file_patterns[i]);
      }
      i += 1;
      s = e + 1;
    }
  ...
  struct giggle_index *gi = giggle_load_with_metadata(
      index_dir_name, m_is_set, block_store_giggle_set_data_handler);
  ...
  if (query_filter_string != NULL) {
    query_filter =
        parse_query_filter_string(gi->metadata_idx, query_filter_string);
  ...
  if (r_is_set == 1) {
    ...
    for (i = 0; i <= len; ++i) {
      if ((regions[i] == ',') || (regions[i] == '\0')) {
        ...
        if (parse_region(region, &chrm, &start, &end) == 0) {
          gqr = giggle_query_with_query_filter(gi, chrm, start, end,
                                               query_filter, gqr);
          free(region);
        } else {
    ...
    while (q_f->input_file_get_next_interval(q_f, &chrm, &chrm_len, &start,
                                             &end, &offset, &line) >= 0) {
      gqr = giggle_query_with_query_filter(gi, chrm, start, end, query_filter,
                                           gqr);
      if ((o_is_set == 1) && (gqr->num_hits > 0)) {
        ...
        if (q_f->type == BED)
          printf("\n");
        int r = print_giggle_query_result(
            gqr, gi, regexs, file_patterns, num_file_patterns, num_intervals,
            mean_interval_size, genome_size, f_is_set, v_is_set, c_is_set,
            s_is_set, o_is_set);
  ...
  int r = print_giggle_query_result(gqr, gi, regexs, file_patterns,
                                    num_file_patterns, num_intervals,
                                    mean_interval_size, genome_size, f_is_set,
                                    v_is_set, c_is_set, s_is_set, o_is_set);
